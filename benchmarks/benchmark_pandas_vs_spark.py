...
def prepare(scale: int, work: Path) -> dict[str, Path]:
    ...
    orders = (
        _read_bronze("orders", ["order_id", "order_date", "status"]).to_pandas().drop_duplicates("order_id")
    )
    ...
    paths = {
        "items": work / f"items_x{scale}.parquet",
        "orders": work / "orders.parquet",
        "products": work / "products.parquet",
    }
...
def run_pandas(paths: dict[str, Path]) -> dict:
    ...
    df = items.merge(orders[["order_id", "order_date_t", "status"]], on="order_id", how="left").merge(
        products[["product_id", "unit_cost_t"]], on="product_id", how="left"
    )
    ...
    agg = df.groupby(["product_id", "day"], as_index=False).agg(
        revenue=("line_total", "sum"), margin=("gross_margin", "sum"), units=("quantity_t", "sum")
    )
...
def run_spark(paths: dict[str, Path], spark) -> dict:
    ...
    orders = orders.withColumn(
        "order_date_t", F.to_timestamp("order_date", "yyyy-MM-dd'T'HH:mm:ss'Z'")
    ).select("order_id", "order_date_t", "status")
    products = products.withColumn("unit_cost_t", F.col("unit_cost").cast("double")).select(
        "product_id", "unit_cost_t"
    )
    df = items.join(orders, "order_id", "left").join(F.broadcast(products), "product_id", "left")
    ok = (
        (F.col("quantity_t") > 0)
        & (F.col("unit_price_t") > 0)
        & F.col("order_date_t").isNotNull()
        & F.col("unit_cost_t").isNotNull()
    )
    df = df.withColumn("ok", ok).cache()
    rejected = df.filter(~F.col("ok")).count()
    good = df.filter("ok").withColumn(
        "line_total",
        F.round(F.col("quantity_t") * F.col("unit_price_t") * (1 - F.col("discount_t") / 100), 2),
    )
    good = good.withColumn(
        "gross_margin", F.col("line_total") - F.col("quantity_t") * F.col("unit_cost_t")
    ).withColumn("day", F.date_trunc("day", "order_date_t"))
    agg = good.groupBy("product_id", "day").agg(
        F.sum("line_total").alias("revenue"),
        F.sum("gross_margin").alias("margin"),
        F.sum("quantity_t").alias("units"),
    )
    w2 = (
...
def main() -> int:
    ...
    for scale in scales:
        ...
        best = {}
        for engine in ("pandas", "pyspark"):
            runs = [
                run_pandas(paths) if engine == "pandas" else run_spark(paths, spark) for _ in range(a.repeat)
            ]
        ...
        rows.append(
            {
                ...
                "pyspark_seconds": s["seconds"],
                "pandas_rows_per_s": int(p["rows_in"] / p["seconds"]),
                "pyspark_rows_per_s": int(s["rows_in"] / s["seconds"]),
            }
        )
        print(
            f"scale x{scale}: rows={p['rows_in']:,} pandas={p['seconds']}s pyspark={s['seconds']}s",
            flush=True,
        )
    stop_spark()
    ...
    doc = {
        ...
        "host": {
            ...
            "spark_writer": settings.effective_spark_writer,
            "spark_startup_seconds": spark_startup,
        },
    ...
    for r in rows:
        print(
            f"| x{r['scale']} | {r['rows_in']:,} | {r['pandas_seconds']} | {r['pyspark_seconds']} | {r['pandas_rows_per_s']:,} | {r['pyspark_rows_per_s']:,} |"
        )
